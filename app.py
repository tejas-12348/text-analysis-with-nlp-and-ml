...
def transform_text(txt):
    transformed_txt = tfidf1.transform([txt])
...
x = tfidf1.fit_transform(df1["Text"])
y = df1["Label"]
...
if rad == "Spam or Ham Detection":
    ...
    if st.button("Predict"):
        if sent1 == "":
            st.sidebar.subheader("Please enter the text to proceed.")
        else:
...
def create_sentiment_plot(sentiment_results):
    df = pd.DataFrame(sentiment_results, columns=['Text', 'Sentiment'])
    fig = px.bar(df, x='Text', y='Sentiment', title='Sentiment Analysis Results')
    return fig
...
if rad=="Sentiment Analysis":
    ...
    if st.button("Predict"):
        if sent2 == "":
            ...
            vector_sent2 = tfidf2.transform([sent2])
            prediction2 = model2.predict(vector_sent2)[0]
            sentiment_results = [{'Text': sent2, 'Sentiment': prediction2}]
            fig = create_sentiment_plot(sentiment_results)
            st.plotly_chart(fig)
...
if rad=="Stress Detection":
    ...
    if st.button("Predict"):
        if sent3 == "":
            ...
            vector_sent3=tfidf3.transform([sent3])
...
if rad=="Hate and Offensive Content Detection":
    ...
    if st.button("Predict"):
        if sent4 == "":
            ...
            vector_sent4=tfidf4.transform([sent4])
...
def spam_detection_page():
    ...
    if st.button("Predict"):
        if sent1 == "":
            ...
            try:
                transformed_sent1 = transform_text(sent1, tfidf1)
                prediction1 = model1.predict(transformed_sent1)[0]
